...
const SubmitButton: React.FC<SubmitButtonProps> = ({ onClick, text, fullWidth=false }) => {
  return (
    <button
      onClick={onClick}
      className={`${fullWidth ? "w-full":"w-3xs"} px-5 py-3 bg-linear-90 from-secondary to-primary hover:bg-secondary transition-all duration-600 transform active:scale-95 text-light font-kanit font-medium text-lg rounded-4xl`}
